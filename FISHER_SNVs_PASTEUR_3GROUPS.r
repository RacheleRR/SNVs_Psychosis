...
# CREATE DATAFRAME TO BE USED FOR STATISTICS 
    ...
    expanded_df <- df %>%
        ...
        filter(group %in% c("FEP", "Non_Converter", "Converter")) %>%  # Remove mixed/NA
    ...
    groups <- c("FEP", "Non_Converter", "Converter")
    ...
    result_df <- data.frame(
        ...
        Non_Converter = c(
            ...
            FEP = c(
            num_non_pathogenic_pLi_variants$num_variants[num_non_pathogenic_pLi_variants$group == "FEP"],
            num_individuals_with_non_pathogenic_pLi$num_individuals[num_individuals_with_non_pathogenic_pLi$group == "FEP"],  # Corrected column
            num_individuals_unique_with_non_pathogenic_pLi$unique_individuals[num_individuals_unique_with_non_pathogenic_pLi$group == "FEP"],
            num_brain_variants$num_brain_variants[num_brain_variants$group == "FEP"],
            num_individuals_with_brain_variants$num_individuals_brain[num_individuals_with_brain_variants$group == "FEP"],  # Corrected column
            num_individuals_unique_with_brain_variants$unique_individuals_brain[num_individuals_unique_with_brain_variants$group == "FEP"],
            num_brain_filter_ntpm_variants$num_brain_filter_ntpm_variants[num_brain_filter_ntpm_variants$group == "FEP"],
            num_individuals_with_brain_filter_ntpm_variants$num_individuals_brain_filter_ntpm[num_individuals_with_brain_filter_ntpm_variants$group == "FEP"],  # Corrected column
            num_individuals_unique_with_brain_filter_ntpm_variants$unique_individuals_brain_filter_ntpm[num_individuals_unique_with_brain_filter_ntpm_variants$group == "FEP"],
            num_schema_pval_variants$num_schema_pval_variants[num_schema_pval_variants$group == "FEP"],
            num_individuals_with_schema_pval_variants$num_individuals_schema_pval[num_individuals_with_schema_pval_variants$group == "FEP"],  # Corrected column
            num_individuals_unique_with_schema_pval_variants$unique_individuals_schema_pval[num_individuals_unique_with_schema_pval_variants$group == "FEP"],
            num_bipolar_variants$num_bipolar_variants[num_bipolar_variants$group == "FEP"],
            num_individuals_with_bipolar_variants$num_individuals_bipolar[num_individuals_with_bipolar_variants$group == "FEP"],  # Corrected column
            num_individuals_unique_with_bipolar_variants$unique_individuals_bipolar[num_individuals_unique_with_bipolar_variants$group == "FEP"],
            num_sfari_non_syndromic_variants$num_sfari_non_syndromic_variants[num_sfari_non_syndromic_variants$group == "FEP"],
            num_individuals_with_sfari_non_syndromic_variants$num_individuals_sfari_non_syndromic[num_individuals_with_sfari_non_syndromic_variants$group == "FEP"],  # Corrected column
            num_individuals_unique_with_sfari_non_syndromic_variants$unique_individuals_sfari_non_syndromic[num_individuals_unique_with_sfari_non_syndromic_variants$group == "FEP"],
            num_schema_or_variants$num_schema_or_variants[num_schema_or_variants$group == "FEP"],
            num_individuals_with_schema_or_variants$num_individuals_schema_or[num_individuals_with_schema_or_variants$group == "FEP"],  # Corrected column
            num_individuals_unique_with_schema_or_variants$unique_individuals_schema_or[num_individuals_unique_with_schema_or_variants$group == "FEP"],
            num_genes$num_genes[num_genes$group == "FEP"],
            num_brain_genes$num_brain_genes[num_brain_genes$group == "FEP"],
            num_brain_filter_ntpm_genes$num_brain_filter_ntpm_genes[num_brain_filter_ntpm_genes$group == "FEP"],
            num_schema_pval_genes$num_schema_pval_genes[num_schema_pval_genes$group == "FEP"],
            num_bipolar_genes$num_bipolar_genes[num_bipolar_genes$group == "FEP"],
            num_sfari_non_syndromic_genes$num_sfari_non_syndromic_genes[num_sfari_non_syndromic_genes$group == "FEP"],
            num_schema_or_genes$num_schema_or_genes[num_schema_or_genes$group == "FEP"]  
...
rate_confront_df <- bind_rows(rate_confront, .id = "Test") %>%
    select(Test, Row, FEP_Rate, Converter_Rate, Non_Converter_Rate)
...
parallel_plot_ind <- pairwise_results_ind_df %>%
    ...
    geom_line(aes(group = interaction(Test, Row), color = Row), 
              alpha = 0.3) +
    geom_point(aes(color = Row, size = -log10(p_adj_per_test)), 
               alpha = 0.7) +
