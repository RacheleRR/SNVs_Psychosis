...
complete_df_Missense <- complete_df_Missense %>% mutate(num_group =  ifelse(group %in% "Case", 1, 0))
complete_df_MPC_only <- complete_df_MPC_only %>% mutate(num_group =  ifelse(group %in% "Case", 1, 0))
complete_df_PTV_canonic <- complete_df_PTV_canonic %>% mutate(num_group =  ifelse(group %in% "Case", 1, 0))
complete_df_PTV <- complete_df_PTV %>% mutate(num_group =  ifelse(group %in% "Case", 1, 0))
...
linear_model <- function(data) {
  results <- purrr::map_dfr(
    ...
    .f = function(col_name) {
      formula <- as.formula(paste("num_group ~", col_name))
      model <- glm(formula, data = data, family = binomial)
      
      tidy_model <- broom::tidy(model, conf.int = TRUE, exponentiate = TRUE) %>%
        dplyr::mutate(predictor = col_name)
  ...
  results <- results %>%
    ...
    ) %>% filter(term != "(Intercept)") %>% select(!predictor)
  
  return(results)
}

Missense_results <- linear_model(complete_df_Missense)
Missense_canonical_results <- linear_model(complete_df_Missense_canonical)
MPC_only_results <- linear_model(complete_df_MPC_only)
PTV_canonic_results <- linear_model(complete_df_PTV_canonic)
PTV_results <- linear_model(complete_df_PTV)

all_results <- bind_rows(
  Missense_results %>% mutate(variant_type = "Missense"),
  Missense_canonical_results %>% mutate(variant_type = "Missense_canonical"),
  MPC_only_results %>% mutate(variant_type = "MPC_only"),
  PTV_canonic_results %>% mutate(variant_type = "PTV_canonic"),
  PTV_results %>% mutate(variant_type = "PTV")
)
...
zero_poisson <- function(data) {
  results <- purrr::map_dfr(
    .x = names(data)[grepl("Number_of_", names(data))],
    .f = function(col_name) {
      formula <- as.formula(paste(col_name, "~ num_group | 1"))
...
volcano_glm <- all_results %>%
    ...
    ggplot(aes(x = log2_OR, y = neg_log10_p)) +
    geom_point(aes(shape = variant_type, color = term), alpha = 0.7, size = 3) +
    geom_hline(yintercept = -log10(0.05), linetype = "dashed", color = "red") +
    geom_vline(xintercept = 0, linetype = "dashed", color = "grey50") +
    geom_label_repel(aes(label = Label), size = 2.5, box.padding = 0.5, max.overlaps = 20) +
    labs(
...
ggsave("volcano_glm_plot.png", plot = volcano_glm, width = 10, height = 6)

# Save the results to CSV files
write.csv(all_poisson_results, "all_poisson_results.csv", row.names = FALSE)
write.csv(all_results, "all_glm_results.csv", row.names = FALSE)


#TRIES
    glm(num_group ~ Number_of_Variants + Number_of_Brain_Variants + Number_of_Brain_Filter_NTPM_Variants + Number_of_Schema_Pval_Variants + Number_of_Bipolar_Variants + Number_of_Sfari_Non_Syndromic_Variants + Number_of_Schema_Or_Variants, 
                 data = complete_df_Missense_canonical, family = binomial)
